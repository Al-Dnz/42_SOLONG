...
void	set_main_texture_1(t_state *state)
{
	state->player.mlx_img = mlx_xpm_file_to_image(state->mlx,
			"./textures/little_mole_1.xpm", &(state->player.width),
			&(state->player.height));
	state->player.addr = (int *)mlx_get_data_addr(state->player.mlx_img,
			&(state->player.bpp),
			&(state->player.line_len), &(state->player.endian));
	state->floor.mlx_img = mlx_xpm_file_to_image(state->mlx,
			"./textures/floor.xpm", &(state->floor.width),
			&(state->floor.height));
	state->floor.addr = (int *)mlx_get_data_addr(state->floor.mlx_img,
			&(state->floor.bpp), &(state->floor.line_len),
			&(state->floor.endian));
...
void	set_main_texture_2(t_state *state)
{
	state->praticable.mlx_img = mlx_xpm_file_to_image(state->mlx,
			"./textures/grass_krita2.xpm", &(state->praticable.width),
	...
	state->collectible.mlx_img = mlx_xpm_file_to_image(state->mlx,
			"./textures/carot_cartoon.xpm", &(state->collectible.width),
...
void	set_main_texture_3(t_state *state)
{
	state->foe.mlx_img = mlx_xpm_file_to_image(state->mlx,
			"./textures/fox_cartoon.xpm", &(state->foe.width),
			&(state->foe.height));
